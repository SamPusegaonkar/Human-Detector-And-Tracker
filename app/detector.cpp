...
std::vector<std::vector<int>> Detector::GetBoundingBoxes(cv::Mat img) {
    ...
    for (int i = 0; i < detectionMat.rows; i++) {
        ...
        if (confidence > 0.4) {
            int box_x = static_cast<int>(detectionMat.at<float>(i, 3) * img.cols);
            int box_y = static_cast<int>(detectionMat.at<float>(i, 4) * img.rows);
            int box_width = static_cast<int>(detectionMat.at<float>(i, 5) * img.cols - box_x);
            int box_height = static_cast<int>(detectionMat.at<float>(i, 6) * img.rows - box_y);
